        ...
        <table class="display" id="basic-1">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><img class="img-thumbnail" width="100" height="100" src="{{ URL::to('/') }}/server/assets/image/category/{{$item->cate_img}}"></td>
              <td>
                @if($item->status==1)

                <form action="{{URL::to('/LoaiSanPham/disabled/'.$item->cate_id)}}" class="change_status_tri" method="post">
                ...
                @else
                <form action="{{URL::to('/LoaiSanPham/enabled/'.$item->cate_id)}}" class="change_status_tri" method="post">
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
                  <input type="hidden" name="_method" value="put" />
              ...
              <td class="flex-column align-items-center justify-content-around">
                ...
                <a href="{{URL::to('/XoaLoaiSanPham',$item->cate_id)}}" onclick="return confirm('Bạn muốn xóa danh mục này ?')">
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
